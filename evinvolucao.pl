...
idAdjud(Id), retract(idAdjud(Id)), NewId is Id+1, assert(idAdjud(NewId)),
evolucao(adjudicante(Id,Nome,Nif,Morada)).
%evolucao(+adjudicante(nome,123456709,morada)) 
%evolucao(adjudicante(12,nome,123456789,morada))
%adjudicante(Id,nome,123456789,morada)
% solucoes(Inv,+adjudicante::Inv,Linv),teste(Linv).
% solucoes(Inv,+adjudicante(A,B,C,D)::Inv,Linv),teste(Linv).
% solucoes(Inv,+adjudicante(13,nom,123456798,morda)::Inv,Linv),teste(Linv).
%solucoes(Id,adjudicante(Id,_,_,_),L).
evolucao(adjudicataria(Nome,Nif,Morada)):-
    idAdjudicata(Id), retract(idAdjudicata(Id)), NewId is Id+1, assert(idAdjudicata(NewId)),
